...
final class Church_Core_Sermon_Sync_Service
{
    ...
    public function sync_recent_sermons(): array
    {
        ...
        foreach ($videos as $video) {
            ...
            if ($legacy_post_id > 0) {
                ...
                $this->maybe_backfill_scripture_reference($legacy_post_id, $scripture_reference);
                $result['backfilled']++;
                $result['skipped']++;
                continue;
            }
    ...
    private function backfill_existing_video_ids(): int
    {
        ...
        foreach ($post_ids as $post_id) {
            if ((string) get_post_meta((int) $post_id, Church_Core_Sermons::YOUTUBE_VIDEO_ID_META_KEY, true) !== '') {
                continue;
            }
